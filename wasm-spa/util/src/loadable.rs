...
impl<T> PartialEq for Loadable<T>
where
    T: PartialEq,
{
...
impl<T> Loadable<T> {
    ...
    fn custom_view<U, LoadedFn, FailedFn>(
        &self,
        unloaded: Html<U>,
        loading: Html<U>,
        loaded_fn: LoadedFn,
    ...
    pub fn restricted_custom_view<U, LoadedFn, FailedFn>(
        &self,
        unloaded: Html<U>,
    ...
    where
        ...
        FailedFn: Fn(&Option<String>) -> Html<U>,
    {
        self.custom_view(unloaded, loading_type.view(), render_fn, failed_fn)
    }
}
