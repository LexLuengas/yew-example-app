...
impl<T> Clone for FetchResponse<T>
where
    T: Clone,
{
    fn clone(&self) -> Self {
...
fn refresh_jwt(
    ...
    fetch_task_collection: &mut Vec<FetchTask>,
) {
    ...
    let request = Request::get(url.as_str())
        ...
        .header(
            "Authorization",
            format!("{} {}", BEARER, jwt_string).as_str(),
        )
        .body(Nothing)
        .unwrap();
...
impl Debug for Networking {
    fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
        write!(
            f,
            "Networking {{ num_held_tasks: {} }}",
            self.fetch_task_collection.len()
        )
    }
}
...
impl Networking {
    ...
    pub fn fetch<T, U, V>(
        &mut self,
        request: &T,
        response_mapper: fn(FetchResponse<U>) -> V::Message,
        link: &ComponentLink<V>,
    ) where
        T: FetchRequest,
        ...
        V::Message: Default,
    {
        ...
        let handle_response_closure = move |response: Response<Text>| {
            ...
            let message: FetchResponse<U> = match meta.status.into() {
                ...
                404 => FetchResponse::Error(FetchError::ResourceNotFound),
                403 => FetchResponse::Error(FetchError::Forbidden),
                400 => FetchResponse::Error(FetchError::BadRequest),
                200...299 => {
    ...
    pub fn fetch_string<T, V>(
        &mut self,
        request: &T,
        response_mapper: fn(FetchResponse<String>) -> V::Message,
        link: &ComponentLink<V>,
    ) where
        T: FetchRequest,
        V: Component + Renderable<V>,
...
fn make_request<T: FetchRequest>(
    fetch_struct: &mut Networking,
    request: &T,
    ...
    match body_and_method {
        HttpMethod::Get => request_builder.method("GET"),
        HttpMethod::Post(_) => request_builder.method("POST"),
        HttpMethod::Put(_) => request_builder.method("PUT"),
        HttpMethod::Delete => request_builder.method("DELETE"),
    };
    ...
    if let Some(token) = user::get_token_if_valid(&mut fetch_struct.storage_service) {
        refresh_jwt_if_needed(
            &mut fetch_struct.fetch_service,
            &token,
            &mut fetch_struct.fetch_task_collection,
        );
        request_builder.header("Authorization", format!("{} {}", BEARER, token).as_str());
    } else if let Auth::Required = auth_requirement {
        fetch_struct
            .router
            .send(RouterRequest::ChangeRoute(Route::parse("/auth/login")));
        return None; // don't continue
    }

    let body: Option<String> = match body_and_method {
        HttpMethod::Get => None,
        HttpMethod::Post(body) => Some(body),
        HttpMethod::Put(body) => Some(body),
        HttpMethod::Delete => None,
    };

    info!(
        "Sending request to: '{}', with auth: {:?}",
        url, auth_requirement
    );
