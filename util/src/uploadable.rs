    ...
    T: Default,
{
    ...
    pub fn handle_fetch_response(&mut self, fetch_response: FetchResponse<T>) {
        ...
        *self = match fetch_response {
            ...
            Error(_) => {
                ...
                Uploadable::Failed(
                    mem::replace(existing_wrapped_value, T::default()),
                    "Failed".into(),
                )
            }
            Started => {
    ...
    pub fn set_failed(&mut self, msg: &str) {
        ...
        *self = match *self {
            Uploadable::NotUploaded(ref mut t) => {
                Uploadable::Failed(mem::replace(t, T::default()), msg)
            }
            Uploadable::Uploading(ref mut t) => {
                Uploadable::Failed(mem::replace(t, T::default()), msg)
            }
            Uploadable::Failed(ref mut t, _) => {
                Uploadable::Failed(mem::replace(t, T::default()), msg)
            }
        }
    }
    pub fn set_uploading(&mut self) {
        *self = match *self {
            Uploadable::NotUploaded(ref mut t) => {
                Uploadable::Uploading(mem::replace(t, T::default()))
            }
            Uploadable::Uploading(ref mut t) => {
                Uploadable::Uploading(mem::replace(t, T::default()))
            }
